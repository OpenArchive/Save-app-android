...
class SnowbirdFileListFragment : BaseSnowbirdFragment() {
    ...
    private fun openContentPickerSheet() {
        val contentPickerSheet = ContentPickerFragment { mediaType ->
            when (mediaType) {
                ...
                AddMediaType.GALLERY -> galleryLauncher.launch(PickVisualMediaRequest(ActivityResultContracts.PickVisualMedia.ImageAndVideo))
                AddMediaType.FILES -> filePickerLauncher.launch(arrayOf("image/*", "video/*", "audio/*"))
            }
        }
    ...
    private fun launchCamera() {
        if (appConfig.useCustomCamera) {
            ...
            currentPhotoUri = androidx.core.content.FileProvider.getUriForFile(requireContext(), "${requireContext().packageName}.fileprovider", photoFile)
