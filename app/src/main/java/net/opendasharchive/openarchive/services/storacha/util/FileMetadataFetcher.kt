...
class FileMetadataFetcher(private val client: OkHttpClient) {
    ...
    private fun parseFileMetadata(html: String, baseUrl: String): FileMetadata? {
        try {
            ...
            val linkPattern = Pattern.compile("<a\\s+href=\"(?:/ipfs/[^/]+/)?([^\"]+)\">([^<]+)</a>")
            ...
            while (matcher.find()) {
                ...
                val sizePattern = Pattern.compile("([0-9]+(?:\\.[0-9]+)?\\s*[KMGT]?B)", Pattern.CASE_INSENSITIVE)
                val sizeMatcher = sizePattern.matcher(context)
                val fileSize = if (sizeMatcher.find()) {
